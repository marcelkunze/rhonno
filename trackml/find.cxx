...
// Look for seeding points by hit pair combinations in the innnermost layers
vector<pair<int, int> > Tracker::findSeedsPhiTheta()
{
    const int n=5; // Seeding layer combinations
    const int start_list[6][2] = {{0,1}, {11,12}, {4,5}, {0,4}, {0,11}, {18,19}};
...
long Tracker::findTriples(vector<pair<int,int> > seed, vector<triple> &triples) {
    ...
    for (auto &it : seed) {
        ...
        if (_verbose) cout << endl << "findTripes " <<  "{" << index << ","  << l << "," << m << "}" << endl;
        vector<triple> trip;
...
long Tracker::addHits(int p0,int p1,int start,std::vector<triple> &triples)
{
    ...
    const auto edgelist = tracking.edges(start); // Get the list of possible connected modules
