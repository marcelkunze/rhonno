...
public:
    ...
    inline float recall(int i=0) const {if (i<NEIGHBOURS && i>=0) return _recall[i]; else return -1;}
    inline float* recalls() {return _recall;}
    inline void setvolume(int vol) { _volume = vol;}
    inline void setlayer(int lay) { _layer = lay;}
    inline void setmodule(int mod) { _module = mod;}
...
Point Point::circleCenter(const Point &p1,const Point &p2,const Point &p3)
{
    ...
    float offset = pow(p2._x,2) + pow(p2._y,2);
    float bc =   ( pow(p1._x,2) + pow(p1._y,2) - offset )/2.0;
