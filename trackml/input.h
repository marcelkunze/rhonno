...
const char base_path[] = "/Users/marcel/workspace/train_sample/";
...
point truth_pos[150000], truth_mom[150000]; //truth position and momentum
double truth_weight[150000]; //weighting of each hit
long long truth_part[150000]; //particle this hit belongs to
map<long long, double> part_weight; //weighting of each particle
map<long long, std::map<int, double> > metai_weight; //weighting of each particle hit, also adding duplicates

//Empirical field strengh, to scale the momentum
const double Bfield = 1673;

std::set<long long> blacklist;
void readBlacklist() {
    if (filenum < 1000) return;
    char file[1000];
    sprintf(file, "%s/event%09d-blacklist_particles.csv", base_path, filenum);
...
void readTruth() {
    if (filenum < 1000) return;
    char file[1000];
    sprintf(file, "%s/event%09d-truth.csv", base_path, filenum);
    FILE*fp = fopen(file, "r");
...
void readParticles() {
    char file[1000];
    sprintf(file, "%s/event%09d-particles.csv", base_path, filenum);
    FILE*fp = fopen(file, "r");
...
map<long long, point> track_hits; // Find points in hits
...
void readHits() {
    ...
    char file[1000];
    if (filenum >= 1000)
        sprintf(file, "%s/event%09d-hits.csv", base_path, filenum);
    else
...
void readCells() {
    char file[1000];
    if (filenum >= 1000)
        sprintf(file, "%s/event%09d-cells.csv", base_path, filenum);
    else
