...
int findTracks(int nhits, float *x, float *y, float *z, int* labels)
{
    ...
    // Join the remaining tracklets to tracks
    for( int i=0; i<tracklet.size(); i++ ) {
        int farestHit = getFarestHit(tracklet[i], &d[i][0]);
        if (farestHit > 0) {
            for( int t=0; t<tracklet.size(); t++ ) {
                if (tracklet[t][0] == farestHit) {
...
int getFarestHit(vector<int> v, int *d) {
    int dmax = -1;
